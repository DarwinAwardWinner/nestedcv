...
#'   details.
#' @param filterFUN Filter function, e.g. [ttest_filter()] or
#'   [relieff_filter()]. Any function can be provided and is passed `y` and `x`.
#'   Ideally returns a numeric vector with indices of filtered predictors. The
#'   custom function can return a character vector of names of the filtered
#'   predictors, but this will not work with the `penalty.factor` argument in
#'   [nestcv.glmnet()].
...
#' @return An object with S3 class "nestcv.train" \item{call}{the matched call}
...
#'   \item{y}{original response vector} \item{yfinal}{final response vector
#'   (post-balancing)} \item{final_fit}{Final fitted caret model using best tune
#'   parameters} \item{final_vars}{Column names of filtered predictors entering
#'   final model} \item{summary_vars}{Summary statistics of filtered predictors}
...
#'   regression.}
#' @details When `finalCV = TRUE`, the final fit on the whole data using is
#'   performed first. This helps flag errors generated by `caret` such as
#'   missing packages. Parallelisation of the final fit when `finalCV = TRUE` is
#'   performed in `caret`, which uses `foreach`. Since the parallelization of
#'   nested CV uses the future framework (see below), it is recommended to use
#'   [doFuture::registerDoFuture()] to make caret also use future for
#'   parallelization.
#'
#'   Parallelisation is performed on the outer CV folds using the future
#'   framework. But some models use multi-threading which may cause issues in
#'   some circumstances with forked multicore processing. Hence, unless future
#'   is using a sequential plan, multithreading is disabled by default using the
#'   future option `future.fork.multithreading.enable`, which for example
#'   prevents caret methods `"xgbTree"` and `"xgbLinear"` from failing due to
#'   the use of multithreading within multi-processing. This can be overridden
#'   by passing the argument `allow_multithreading`.
#'
#'   Since inner folds are trained using caret, if you follow the recommendation
#'   above to use [doFuture::registerDoFuture()], then you can control the
#'   parallelism at each nesting level using future topologies. (See
#'   `vignette("future-3-topologies")`.)
#'
...
#'   `caret`. See [caret::trainControl()]. LogLoss is arguably more consistent
#'   than Accuracy for tuning parameters in datasets with small sample size.
#'
...
nestcv.train <- function(y, x,
                         ...
                         ...) {
  if (!missing(cv.cores) || !missing(multicore_fork)) {
    warning("Arguments cv.cores and multicore_fork are deprecated. Use future::plan() instead.")
  }
  start <- Sys.time()
...
# Prints a message using shell echo from inside mclapply when run in Rstudio (no
# idea how it interacts with future_lapply)
